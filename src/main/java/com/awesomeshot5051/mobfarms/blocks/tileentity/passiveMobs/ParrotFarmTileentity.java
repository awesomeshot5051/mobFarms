...
public class ParrotFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getParrotSpawnTime(ParrotFarmTileentity farm) {
        SwordType sword = SwordType.fromItem(farm.getSwordType().getItem());
        return (double) Main.SERVER_CONFIG.parrotSpawnTime.get() /
    ...
    public static double getParrotKillTime(ParrotFarmTileentity farm) {
        ...
        return getParrotSpawnTime(farm) + (sword.equals(SwordType.NETHERITE) ? (baseValue * 3.2) :
...
//        BlockBase.playRandomVillagerSound(level, getBlockPos(), SoundEvents.CREEPER_PRIMED);
        ...
        if (timer == getParrotSpawnTime(this)) {
...
//            }
        } else if (timer >= getParrotKillTime(this)) {
