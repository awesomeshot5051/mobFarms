...
/*    0 */ import net.minecraft.world.entity.monster.Strider;
...
/*    0 */ 
/*    0 */ public class StriderFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
/*   37 */   private static final ResourceKey<LootTable> STRIDER_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/strider"));
/*    0 */   
...
/*    0 */   
/*    0 */   public StriderFarmTileentity(BlockPos pos, BlockState state) {
/*   47 */     super((BlockEntityType)ModTileEntities.STRIDER_FARM.get(), ((StriderFarmBlock)ModBlocks.STRIDER_FARM.get()).defaultBlockState(), pos, state);
...
/*    0 */   }
/*    0 */   
/*    0 */   public static int getStriderSpawnTime() {
/*   54 */     return ((Integer)Main.SERVER_CONFIG.striderSpawnTime.get()).intValue() - 200;
/*    0 */   }
/*    0 */   
/*    0 */   public static int getStriderKillTime() {
/*   58 */     return getStriderSpawnTime() + 200;
/*    0 */   }
/*    0 */   
...
/*   74 */     this.timer++;
/*   75 */     setChanged();
/*   77 */     if (this.timer == getStriderSpawnTime()) {
/*   79 */       sync();
/*   80 */     } else if (this.timer > getStriderSpawnTime() && this.timer < getStriderKillTime()) {
/*   81 */       if (this.timer % 20L == 0L);
/*   84 */     } else if (this.timer >= getStriderKillTime()) {
...
/*    0 */     } 
/*  108 */     LootParams.Builder builder = (new LootParams.Builder(serverWorld)).withParameter(LootContextParams.THIS_ENTITY, new Strider(EntityType.STRIDER, this.level)).withParameter(LootContextParams.ORIGIN, new Vec3(this.worldPosition.getX(), this.worldPosition.getY(), this.worldPosition.getZ())).withParameter(LootContextParams.DAMAGE_SOURCE, serverWorld.damageSources().lava());
/*  110 */     LootParams lootContext = builder.create(LootContextParamSets.ENTITY);
/*  112 */     LootTable lootTable = serverWorld.getServer().reloadableRegistries().getLootTable(STRIDER_LOOT_TABLE);
