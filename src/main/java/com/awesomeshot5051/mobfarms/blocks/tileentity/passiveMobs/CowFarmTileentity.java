...
public class CowFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getCowSpawnTime(CowFarmTileentity farm) {
        SwordType sword = SwordType.fromItem(farm.getSwordType().getItem());
        return (double) Main.SERVER_CONFIG.cowSpawnTime.get() /
    ...
    public static double getCowKillTime(CowFarmTileentity farm) {
        ...
        return getCowSpawnTime(farm) + (sword.equals(SwordType.NETHERITE) ? (baseValue * 3.2) :
...
//            }
        } else if (timer >= getCowKillTime(this)) {
