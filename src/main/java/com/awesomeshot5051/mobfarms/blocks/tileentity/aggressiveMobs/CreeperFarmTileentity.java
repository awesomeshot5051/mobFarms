...
public class CreeperFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getCreeperSpawnTime(CreeperFarmTileentity farm) {
        SwordType sword = SwordType.fromItem(farm.getSwordType().getItem());

        return (double) Main.SERVER_CONFIG.creeperSpawnTime.get() /
    ...
    public static double getCreeperExplodeTime(CreeperFarmTileentity farm) {
        SwordType sword = SwordType.fromItem(farm.getSwordType().getItem());
        ...
        return getCreeperSpawnTime(farm) + (sword.equals(SwordType.NETHERITE) ? (baseValue * 3.2) :
...
//        BlockBase.playRandomVillagerSound(level, getBlockPos(), SoundEvents.CREEPER_PRIMED);
        ...
        if (timer == getCreeperSpawnTime(this)) {
...
//            }
        } else if (timer >= getCreeperExplodeTime(this)) {
