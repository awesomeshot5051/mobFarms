...
public class SlimeFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getSlimeSpawnTime(SlimeFarmTileentity farm) {
        SwordType sword = SwordType.fromItem(farm.getSwordType().getItem());
        return (double) Main.SERVER_CONFIG.slimeSpawnTime.get() /
    ...
    public static double getSlimeExplodeTime(SlimeFarmTileentity farm) {
        ...
        return getSlimeSpawnTime(farm) + (sword.equals(SwordType.NETHERITE) ? (baseValue * 3.2) :
...
//        BlockBase.playRandomVillagerSound(level, getBlockPos(), SoundEvents.SLIME_PRIMED);
        ...
        if (timer == getSlimeSpawnTime(this)) {
...
//            }
        } else if (timer >= getSlimeExplodeTime(this)) {
