...
public class GoatFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getGoatSpawnTime(GoatFarmTileentity farm) {
        SwordType sword = SwordType.fromItem(farm.getSwordType().getItem());
        return (double) Main.SERVER_CONFIG.goatSpawnTime.get() /
    ...
    public static double getGoatKillTime(GoatFarmTileentity farm) {
        ...
        return getGoatSpawnTime(farm) + (sword.equals(SwordType.NETHERITE) ? (baseValue * 3.2) :
...
//        BlockBase.playRandomVillagerSound(level, getBlockPos(), SoundEvents.CREEPER_PRIMED);
        ...
        if (timer == getGoatSpawnTime(this)) {
...
//            }
        } else if (timer >= getGoatKillTime(this)) {
