...
/*    0 */ import net.minecraft.world.entity.animal.Salmon;
...
/*    0 */ 
/*    0 */ public class SalmonFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
/*   37 */   private static final ResourceKey<LootTable> SALMON_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/salmon"));
/*    0 */   
...
/*    0 */   
/*    0 */   public SalmonFarmTileentity(BlockPos pos, BlockState state) {
/*   47 */     super((BlockEntityType)ModTileEntities.SALMON_FARM.get(), ((SalmonFarmBlock)ModBlocks.SALMON_FARM.get()).defaultBlockState(), pos, state);
...
/*    0 */   }
/*    0 */   
/*    0 */   public static int getSalmonSpawnTime() {
/*   54 */     return ((Integer)Main.SERVER_CONFIG.salmonSpawnTime.get()).intValue() - 200;
/*    0 */   }
/*    0 */   
/*    0 */   public static int getSalmonKillTime() {
/*   58 */     return getSalmonSpawnTime() + 200;
/*    0 */   }
/*    0 */   
...
/*   74 */     this.timer++;
/*   75 */     setChanged();
/*   77 */     if (this.timer == getSalmonSpawnTime()) {
/*   79 */       sync();
/*   80 */     } else if (this.timer > getSalmonSpawnTime() && this.timer < getSalmonKillTime()) {
/*   81 */       if (this.timer % 20L == 0L);
/*   84 */     } else if (this.timer >= getSalmonKillTime()) {
...
/*    0 */     } 
/*  108 */     LootParams.Builder builder = (new LootParams.Builder(serverWorld)).withParameter(LootContextParams.THIS_ENTITY, new Salmon(EntityType.SALMON, this.level)).withParameter(LootContextParams.ORIGIN, new Vec3(this.worldPosition.getX(), this.worldPosition.getY(), this.worldPosition.getZ())).withParameter(LootContextParams.DAMAGE_SOURCE, serverWorld.damageSources().lava());
/*  110 */     LootParams lootContext = builder.create(LootContextParamSets.ENTITY);
/*  112 */     LootTable lootTable = serverWorld.getServer().reloadableRegistries().getLootTable(SALMON_LOOT_TABLE);
