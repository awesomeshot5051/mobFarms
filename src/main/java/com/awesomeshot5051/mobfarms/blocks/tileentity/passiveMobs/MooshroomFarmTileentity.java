...
public class MooshroomFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getMooshroomSpawnTime(MooshroomFarmTileentity farm) {
        SwordType sword = SwordType.fromItem(farm.getSwordType().getItem());
        return (double) Main.SERVER_CONFIG.mooshroomSpawnTime.get() /
    ...
    public static double getMooshroomKillTime(MooshroomFarmTileentity farm) {
        ...
        return getMooshroomSpawnTime(farm) + (sword.equals(SwordType.NETHERITE) ? (baseValue * 3.2) :
...
//        BlockBase.playRandomVillagerSound(level, getBlockPos(), SoundEvents.CREEPER_PRIMED);
        ...
        if (timer == getMooshroomSpawnTime(this)) {
...
//            }
        } else if (timer >= getMooshroomKillTime(this)) {
