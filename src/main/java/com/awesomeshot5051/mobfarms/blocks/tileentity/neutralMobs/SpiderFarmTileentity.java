...
public class SpiderFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getSpiderSpawnTime(SpiderFarmTileentity farm) {
        SwordType sword = SwordType.fromItem(farm.getSwordType().getItem());
        return (double) Main.SERVER_CONFIG.spiderSpawnTime.get() /
    ...
    public static double getSpiderKillTime(SpiderFarmTileentity farm) {
        ...
        return getSpiderSpawnTime(farm) + (sword.equals(SwordType.NETHERITE) ? (baseValue * 3.2) :
...
//        BlockBase.playRandomVillagerSound(level, getBlockPos(), SoundEvents.CREEPER_PRIMED);
        ...
        if (timer == getSpiderSpawnTime(this)) {
...
//            }
        } else if (timer >= getSpiderKillTime(this)) {
...
//            // VillagerBlockBase.playVillagerSound(level, getBlockPos(), SoundEvents.CREEPER_DEATH);
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(Items.STRING, dropCount));
        if (SwordEnchantments.getEnchantmentStatus(swordEnchantments, Enchantments.LOOTING) ? serverWorld.random.nextFloat() >= (float) 2 / 3 : serverWorld.random.nextFloat() >= (float) 1 / 3) {
