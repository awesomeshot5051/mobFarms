...
/*    0 */ import net.minecraft.world.entity.monster.ZombifiedPiglin;
...
/*    0 */ 
/*    0 */ public class ZombifiedPiglinFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
/*   38 */   private static final ResourceKey<LootTable> ZOMBIFIEDPIGLIN_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/zombified_piglin"));
/*    0 */   
...
/*    0 */   
/*    0 */   public ZombifiedPiglinFarmTileentity(BlockPos pos, BlockState state) {
/*   46 */     super((BlockEntityType)ModTileEntities.ZOMBIFIED_PIGLIN_FARM.get(), ((ZombifiedPiglinFarmBlock)ModBlocks.ZOMBIFIED_PIGLIN_FARM.get()).defaultBlockState(), pos, state);
...
/*    0 */   }
/*    0 */   
/*    0 */   public static int getZombifiedPiglinSpawnTime() {
/*   53 */     return ((Integer)Main.SERVER_CONFIG.zombifiedPiglinSpawnTime.get()).intValue() - 200;
/*    0 */   }
/*    0 */   
/*    0 */   public static int getZombifiedPiglinKillTime() {
/*   57 */     return getZombifiedPiglinSpawnTime() + 200;
/*    0 */   }
/*    0 */   
...
/*   69 */     this.timer++;
/*   70 */     setChanged();
/*   72 */     if (this.timer == getZombifiedPiglinSpawnTime()) {
/*   75 */       sync();
/*   80 */     } else if (this.timer >= getZombifiedPiglinKillTime()) {
...
/*    0 */     } 
/*  105 */     LootParams.Builder builder = (new LootParams.Builder(serverWorld)).withParameter(LootContextParams.THIS_ENTITY, new ZombifiedPiglin(EntityType.ZOMBIFIED_PIGLIN, this.level)).withParameter(LootContextParams.ORIGIN, new Vec3(this.worldPosition.getX(), this.worldPosition.getY(), this.worldPosition.getZ())).withParameter(LootContextParams.DAMAGE_SOURCE, serverWorld.damageSources().lava());
/*  107 */     LootParams lootContext = builder.create(LootContextParamSets.ENTITY);
/*  109 */     LootTable lootTable = serverWorld.getServer().reloadableRegistries().getLootTable(ZOMBIFIEDPIGLIN_LOOT_TABLE);
