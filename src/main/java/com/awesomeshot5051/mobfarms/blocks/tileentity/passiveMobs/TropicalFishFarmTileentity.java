...
/*    0 */ import net.minecraft.world.entity.animal.TropicalFish;
...
/*    0 */ 
/*    0 */ public class TropicalFishFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
/*   37 */   private static final ResourceKey<LootTable> TROPICAL_FISH_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/tropical_fish"));
/*    0 */   
...
/*    0 */   
/*    0 */   public TropicalFishFarmTileentity(BlockPos pos, BlockState state) {
/*   47 */     super((BlockEntityType)ModTileEntities.TROPICAL_FISH_FARM.get(), ((TropicalFishFarmBlock)ModBlocks.TROPICAL_FISH_FARM.get()).defaultBlockState(), pos, state);
...
/*    0 */   }
/*    0 */   
/*    0 */   public static int getTropicalFishSpawnTime() {
/*   54 */     return ((Integer)Main.SERVER_CONFIG.tropicalFishSpawnTime.get()).intValue() - 200;
/*    0 */   }
/*    0 */   
/*    0 */   public static int getTropicalFishKillTime() {
/*   58 */     return getTropicalFishSpawnTime() + 200;
/*    0 */   }
/*    0 */   
...
/*   74 */     this.timer++;
/*   75 */     setChanged();
/*   77 */     if (this.timer == getTropicalFishSpawnTime()) {
/*   79 */       sync();
/*   80 */     } else if (this.timer > getTropicalFishSpawnTime() && this.timer < getTropicalFishKillTime()) {
/*   81 */       if (this.timer % 20L == 0L);
/*   84 */     } else if (this.timer >= getTropicalFishKillTime()) {
...
/*    0 */     } 
/*  108 */     LootParams.Builder builder = (new LootParams.Builder(serverWorld)).withParameter(LootContextParams.THIS_ENTITY, new TropicalFish(EntityType.TROPICAL_FISH, this.level)).withParameter(LootContextParams.ORIGIN, new Vec3(this.worldPosition.getX(), this.worldPosition.getY(), this.worldPosition.getZ())).withParameter(LootContextParams.DAMAGE_SOURCE, serverWorld.damageSources().lava());
/*  110 */     LootParams lootContext = builder.create(LootContextParamSets.ENTITY);
/*  112 */     LootTable lootTable = serverWorld.getServer().reloadableRegistries().getLootTable(TROPICAL_FISH_LOOT_TABLE);
