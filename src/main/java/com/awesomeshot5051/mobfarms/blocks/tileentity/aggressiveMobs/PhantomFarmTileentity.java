...
/*    0 */ import net.minecraft.world.entity.monster.Phantom;
...
/*    0 */ 
/*    0 */ public class PhantomFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
/*   38 */   private static final ResourceKey<LootTable> PHANTOM_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/phantom"));
/*    0 */   
...
/*    0 */   
/*    0 */   public PhantomFarmTileentity(BlockPos pos, BlockState state) {
/*   47 */     super((BlockEntityType)ModTileEntities.PHANTOM_FARM.get(), ((PhantomFarmBlock)ModBlocks.PHANTOM_FARM.get()).defaultBlockState(), pos, state);
...
/*    0 */   }
/*    0 */   
/*    0 */   public static int getPhantomSpawnTime() {
/*   54 */     return ((Integer)Main.SERVER_CONFIG.phantomSpawnTime.get()).intValue() - 80;
/*    0 */   }
/*    0 */   
/*    0 */   public static int getPhantomExplodeTime() {
/*   58 */     return getPhantomSpawnTime() + 80;
/*    0 */   }
/*    0 */   
...
/*   70 */     this.timer++;
/*   71 */     setChanged();
/*   73 */     if (this.timer == getPhantomSpawnTime()) {
/*   76 */       sync();
/*   81 */     } else if (this.timer >= getPhantomExplodeTime()) {
...
/*    0 */     } 
/*  106 */     LootParams.Builder builder = (new LootParams.Builder(serverWorld)).withParameter(LootContextParams.THIS_ENTITY, new Phantom(EntityType.PHANTOM, this.level)).withParameter(LootContextParams.ORIGIN, new Vec3(this.worldPosition.getX(), this.worldPosition.getY(), this.worldPosition.getZ())).withParameter(LootContextParams.DAMAGE_SOURCE, serverWorld.damageSources().explosion(null));
/*  108 */     LootParams lootContext = builder.create(LootContextParamSets.ENTITY);
/*  110 */     LootTable lootTable = serverWorld.getServer().reloadableRegistries().getLootTable(PHANTOM_LOOT_TABLE);
/*  112 */     return Collections.singletonList(new ItemStack((ItemLike)Items.PHANTOM_MEMBRANE, 3));
/*    0 */   }
/*    0 */   
...
/*  123 */     ContainerHelper.saveAllItems(compound, this.inventory, false, provider);
/*  124 */     compound.putLong("Timer", this.timer);
/*    0 */   }
/*    0 */   
/*    0 */   protected void loadAdditional(CompoundTag compound, HolderLookup.Provider provider) {
/*  129 */     ContainerHelper.loadAllItems(compound, this.inventory, provider);
/*  130 */     this.timer = compound.getLong("Timer");
/*  131 */     super.loadAdditional(compound, provider);
/*    0 */   }
/*    0 */   
/*    0 */   public IItemHandler getItemHandler() {
/*  135 */     return (IItemHandler)this.outputItemHandler;
/*    0 */   }
/*    0 */ }
