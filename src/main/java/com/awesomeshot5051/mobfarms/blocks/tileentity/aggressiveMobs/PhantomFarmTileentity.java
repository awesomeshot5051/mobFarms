...
public class PhantomFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getPhantomSpawnTime(PhantomFarmTileentity farm) {
        SwordType sword = SwordType.fromItem(farm.getSwordType().getItem());
        return (double) Main.SERVER_CONFIG.phantomSpawnTime.get() /
    ...
    public static double getPhantomExplodeTime(PhantomFarmTileentity farm) {
        ...
        return getPhantomSpawnTime(farm) + (sword.equals(SwordType.NETHERITE) ? (baseValue * 3.2) :
...
//        BlockBase.playRandomVillagerSound(level, getBlockPos(), SoundEvents.PHANTOM_PRIMED);
        ...
        if (timer == getPhantomSpawnTime(this)) {
...
//            }
        } else if (timer >= getPhantomExplodeTime(this)) {
...
//            // VillagerBlockBase.playVillagerSound(level, getBlockPos(), SoundEvents.PHANTOM_DEATH);
    ...
    private List<ItemStack> getDrops() {
        ...
        return Collections.singletonList(new ItemStack(Items.PHANTOM_MEMBRANE, dropCount));
    }
