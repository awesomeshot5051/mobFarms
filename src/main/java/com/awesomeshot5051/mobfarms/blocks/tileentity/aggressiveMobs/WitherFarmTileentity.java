...
public class WitherFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getWitherSpawnTime(WitherFarmTileentity farm) {
        SwordType sword = SwordType.fromItem(farm.getSwordType().getItem());
        return (double) Main.SERVER_CONFIG.witherSpawnTime.get() /
    ...
    public static double getWitherExplodeTime(WitherFarmTileentity farm) {
        ...
        return getWitherSpawnTime(farm) + (sword.equals(SwordType.NETHERITE) ? (baseValue * 3.2) :
...
//        BlockBase.playRandomVillagerSound(level, getBlockPos(), SoundEvents.WITHER_PRIMED);
        ...
        if (timer == getWitherSpawnTime(this)) {
...
//            }
        } else if (timer >= getWitherExplodeTime(this)) {
