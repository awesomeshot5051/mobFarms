...
public class IllusionerFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getIllusionerSpawnTime(IllusionerFarmTileentity farm) {
        SwordType sword = SwordType.fromItem(farm.getSwordType().getItem());
        return (double) Main.SERVER_CONFIG.illusionerSpawnTime.get() /
    ...
    public static double getIllusionerExplodeTime(IllusionerFarmTileentity farm) {
        ...
        return getIllusionerSpawnTime(farm) + (sword.equals(SwordType.NETHERITE) ? (baseValue * 3.2) :
...
//        BlockBase.playRandomVillagerSound(level, getBlockPos(), SoundEvents.ILLUSIONER_PRIMED);
        ...
        if (timer == getIllusionerSpawnTime(this)) {
...
//            }
        } else if (timer >= getIllusionerExplodeTime(this)) {
...
//            // VillagerBlockBase.playVillagerSound(level, getBlockPos(), SoundEvents.ILLUSIONER_DEATH);
    ...
    private List<ItemStack> getDrops() {
        ...
        if (SwordEnchantments.getEnchantmentStatus(swordEnchantments, Enchantments.LOOTING) ? serverWorld.random.nextFloat() >= .115 : serverWorld.random.nextFloat() >= .085) {
            drops.add(new ItemStack(Items.CROSSBOW));
        }
