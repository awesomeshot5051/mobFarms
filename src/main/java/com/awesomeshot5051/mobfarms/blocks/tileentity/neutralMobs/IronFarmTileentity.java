...
/*    0 */ import net.minecraft.world.entity.animal.IronGolem;
...
/*    0 */ 
/*    0 */ public class IronFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
/*   37 */   private static final ResourceKey<LootTable> GOLEM_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/iron_golem"));
/*    0 */   
...
/*    0 */   
/*    0 */   public IronFarmTileentity(BlockPos pos, BlockState state) {
/*   47 */     super((BlockEntityType)ModTileEntities.IRON_FARM.get(), ((IronFarmBlock)ModBlocks.IRON_FARM.get()).defaultBlockState(), pos, state);
...
/*    0 */   }
/*    0 */   
/*    0 */   public static int getGolemSpawnTime() {
/*   54 */     return ((Integer)Main.SERVER_CONFIG.golemSpawnTime.get()).intValue() - 200;
/*    0 */   }
/*    0 */   
/*    0 */   public static int getGolemKillTime() {
/*   58 */     return getGolemSpawnTime() + 200;
/*    0 */   }
/*    0 */   
...
/*   76 */     this.timer++;
/*   77 */     setChanged();
/*   79 */     if (this.timer == getGolemSpawnTime()) {
/*   81 */       sync();
/*   82 */     } else if (this.timer > getGolemSpawnTime() && this.timer < getGolemKillTime()) {
/*   83 */       if (this.timer % 20L == 0L);
/*   86 */     } else if (this.timer >= getGolemKillTime()) {
/*   88 */       for (ItemStack drop : getDrops()) {
/*   89 */         for (int i = 0; i < this.itemHandler.getSlots(); i++) {
/*   90 */           drop = this.itemHandler.insertItem(i, drop, false);
/*   91 */           if (drop.isEmpty())
/*    0 */             break; 
/*    0 */         } 
/*    0 */       } 
...
/*  104 */     Level level = this.level;
/*  104 */     if (level instanceof ServerLevel) {
/*  104 */       serverWorld = (ServerLevel)level;
/*    0 */     } else {
/*  105 */       return Collections.emptyList();
/*    0 */     } 
/*  111 */     LootParams.Builder builder = (new LootParams.Builder(serverWorld)).withParameter(LootContextParams.THIS_ENTITY, new IronGolem(EntityType.IRON_GOLEM, this.level)).withParameter(LootContextParams.ORIGIN, new Vec3(this.worldPosition.getX(), this.worldPosition.getY(), this.worldPosition.getZ())).withParameter(LootContextParams.DAMAGE_SOURCE, serverWorld.damageSources().lava());
/*  113 */     LootParams lootContext = builder.create(LootContextParamSets.ENTITY);
/*  115 */     LootTable lootTable = serverWorld.getServer().reloadableRegistries().getLootTable(GOLEM_LOOT_TABLE);
/*  117 */     return (List<ItemStack>)lootTable.getRandomItems(lootContext);
/*    0 */   }
/*    0 */   
/*    0 */   public Container getOutputInventory() {
/*  121 */     return (Container)new ItemListInventory(this.inventory, this::setChanged);
/*    0 */   }
/*    0 */   
/*    0 */   protected void saveAdditional(CompoundTag compound, HolderLookup.Provider provider) {
/*  126 */     super.saveAdditional(compound, provider);
