...
/*    0 */ import net.minecraft.world.entity.animal.Cod;
...
/*    0 */ 
/*    0 */ public class CodFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
/*   39 */   private static final ResourceKey<LootTable> COD_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/cod"));
/*    0 */   
...
/*    0 */   
/*    0 */   public CodFarmTileentity(BlockPos pos, BlockState state) {
/*   47 */     super((BlockEntityType)ModTileEntities.COD_FARM.get(), ((CodFarmBlock)ModBlocks.COD_FARM.get()).defaultBlockState(), pos, state);
...
/*    0 */   }
/*    0 */   
/*    0 */   public static int getCodSpawnTime() {
/*   54 */     return ((Integer)Main.SERVER_CONFIG.codSpawnTime.get()).intValue() - 200;
/*    0 */   }
/*    0 */   
/*    0 */   public static int getCodKillTime() {
/*   58 */     return getCodSpawnTime() + 200;
/*    0 */   }
/*    0 */   
...
/*   70 */     this.timer++;
/*   71 */     setChanged();
/*   73 */     if (this.timer == getCodSpawnTime()) {
/*   76 */       sync();
/*   81 */     } else if (this.timer >= getCodKillTime()) {
...
/*    0 */     } 
/*  106 */     LootParams.Builder builder = (new LootParams.Builder(serverWorld)).withParameter(LootContextParams.THIS_ENTITY, new Cod(EntityType.COD, this.level)).withParameter(LootContextParams.ORIGIN, new Vec3(this.worldPosition.getX(), this.worldPosition.getY(), this.worldPosition.getZ())).withParameter(LootContextParams.DAMAGE_SOURCE, serverWorld.damageSources().lava());
/*  108 */     LootParams lootContext = builder.create(LootContextParamSets.ENTITY);
/*  110 */     LootTable lootTable = serverWorld.getServer().reloadableRegistries().getLootTable(COD_LOOT_TABLE);
/*  112 */     return Collections.singletonList(new ItemStack((ItemLike)Items.COOKED_COD, 3));
/*    0 */   }
/*    0 */   
