...
public class PiglinFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getPiglinSpawnTime(PiglinFarmTileentity farm) {
        SwordType sword = SwordType.fromItem(farm.getSwordType().getItem());
        return (double) Main.SERVER_CONFIG.piglinSpawnTime.get() /
    ...
    public static double getPiglinKillTime(PiglinFarmTileentity farm) {
        ...
        return getPiglinSpawnTime(farm) + (sword.equals(SwordType.NETHERITE) ? (baseValue * 3.2) :
...
//        BlockBase.playRandomVillagerSound(level, getBlockPos(), SoundEvents.CREEPER_PRIMED);
        ...
        if (timer == getPiglinSpawnTime(this)) {
...
//            }
        } else if (timer >= getPiglinKillTime(this)) {
