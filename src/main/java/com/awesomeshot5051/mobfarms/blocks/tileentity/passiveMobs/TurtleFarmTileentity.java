...
/*    0 */ import net.minecraft.world.entity.animal.Turtle;
...
/*    0 */ 
/*    0 */ public class TurtleFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
/*   37 */   private static final ResourceKey<LootTable> TURTLE_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/turtle"));
/*    0 */   
...
/*    0 */   
/*    0 */   public TurtleFarmTileentity(BlockPos pos, BlockState state) {
/*   47 */     super((BlockEntityType)ModTileEntities.TURTLE_FARM.get(), ((TurtleFarmBlock)ModBlocks.TURTLE_FARM.get()).defaultBlockState(), pos, state);
...
/*    0 */   }
/*    0 */   
/*    0 */   public static int getTurtleSpawnTime() {
/*   54 */     return ((Integer)Main.SERVER_CONFIG.turtleSpawnTime.get()).intValue() - 200;
/*    0 */   }
/*    0 */   
/*    0 */   public static int getTurtleKillTime() {
/*   58 */     return getTurtleSpawnTime() + 200;
/*    0 */   }
/*    0 */   
...
/*   74 */     this.timer++;
/*   75 */     setChanged();
/*   77 */     if (this.timer == getTurtleSpawnTime()) {
/*   79 */       sync();
/*   80 */     } else if (this.timer > getTurtleSpawnTime() && this.timer < getTurtleKillTime()) {
/*   81 */       if (this.timer % 20L == 0L);
/*   84 */     } else if (this.timer >= getTurtleKillTime()) {
...
/*    0 */     } 
/*  108 */     LootParams.Builder builder = (new LootParams.Builder(serverWorld)).withParameter(LootContextParams.THIS_ENTITY, new Turtle(EntityType.TURTLE, this.level)).withParameter(LootContextParams.ORIGIN, new Vec3(this.worldPosition.getX(), this.worldPosition.getY(), this.worldPosition.getZ())).withParameter(LootContextParams.DAMAGE_SOURCE, serverWorld.damageSources().lava());
/*  110 */     LootParams lootContext = builder.create(LootContextParamSets.ENTITY);
/*  112 */     LootTable lootTable = serverWorld.getServer().reloadableRegistries().getLootTable(TURTLE_LOOT_TABLE);
