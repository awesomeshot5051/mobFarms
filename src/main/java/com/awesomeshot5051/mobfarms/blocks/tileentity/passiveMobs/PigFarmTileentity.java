...
public class PigFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getPigSpawnTime(PigFarmTileentity farm) {
        SwordType sword = SwordType.fromItem(farm.getSwordType().getItem());
        return (double) Main.SERVER_CONFIG.pigSpawnTime.get() /
    ...
    public static double getPorkKillTime(PigFarmTileentity farm) {
        ...
        return getPigSpawnTime(farm) + (sword.equals(SwordType.NETHERITE) ? (baseValue * 3.2) :
...
//            }
        } else if (timer >= getPorkKillTime(this)) {
