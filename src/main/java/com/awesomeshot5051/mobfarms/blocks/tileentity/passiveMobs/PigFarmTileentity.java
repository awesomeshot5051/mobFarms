...
/*    0 */ import net.minecraft.world.entity.animal.Pig;
...
/*    0 */ 
/*    0 */ public class PigFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
/*   39 */   private static final ResourceKey<LootTable> PIG_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/pig"));
/*    0 */   
...
/*    0 */   
/*    0 */   public PigFarmTileentity(BlockPos pos, BlockState state) {
/*   47 */     super((BlockEntityType)ModTileEntities.PIG_FARM.get(), ((PigFarmBlock)ModBlocks.PIG_FARM.get()).defaultBlockState(), pos, state);
...
/*    0 */   }
/*    0 */   
/*    0 */   public static int getPigSpawnTime() {
/*   54 */     return ((Integer)Main.SERVER_CONFIG.pigSpawnTime.get()).intValue() - 200;
/*    0 */   }
/*    0 */   
/*    0 */   public static int getPorkKillTime() {
/*   58 */     return getPigSpawnTime() + 200;
/*    0 */   }
/*    0 */   
...
/*   70 */     this.timer++;
/*   71 */     setChanged();
/*   73 */     if (this.timer == getPigSpawnTime()) {
/*   76 */       sync();
/*   81 */     } else if (this.timer >= getPorkKillTime()) {
...
/*    0 */     } 
/*  106 */     LootParams.Builder builder = (new LootParams.Builder(serverWorld)).withParameter(LootContextParams.THIS_ENTITY, new Pig(EntityType.PIG, this.level)).withParameter(LootContextParams.ORIGIN, new Vec3(this.worldPosition.getX(), this.worldPosition.getY(), this.worldPosition.getZ())).withParameter(LootContextParams.DAMAGE_SOURCE, serverWorld.damageSources().lava());
/*  108 */     LootParams lootContext = builder.create(LootContextParamSets.ENTITY);
/*  110 */     LootTable lootTable = serverWorld.getServer().reloadableRegistries().getLootTable(PIG_LOOT_TABLE);
/*  112 */     return Collections.singletonList(new ItemStack((ItemLike)Items.COOKED_PORKCHOP, 3));
/*    0 */   }
/*    0 */   
